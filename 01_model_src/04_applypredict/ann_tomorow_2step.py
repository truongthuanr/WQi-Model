...
input_col_tomorrow = ['Giống tôm','area','Độ cứng','TDS','pH','Loại ao','Công nghệ nuôi','Tuổi tôm','Độ kiềm tdpred']
...
def Ann_random_cv(
    ...
    for train_idx, test_idx in ss.split(X_today):
        ...
        Xval_scaled = scaler_X.transform(Xval_tomorrow)

        yval_pred_scaled = model1.predict(Xval_scaled).flatten()
        yval_pred = scaler_y.inverse_transform(yval_pred_scaled.reshape(-1, 1)).flatten()
