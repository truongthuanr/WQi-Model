...
def setup_logger(log_path):
    logger = logging.getLogger('TimeSeriesANN')
...
def create_lag_features_and_target_tomorrow(
    ...
    """
    Create lag features for the provided column along with the next-day target.
...
def Ann_random_cv(
    ...
    for train_index, test_index in splitter.split(X):
        ...
        model.add(Dense(72, kernel_initializer='he_uniform', activation='relu'))
        ...
        y_pred_scaled = model.predict(X_scaler.transform(X_test)).reshape(-1, 1)
...
def plot_predictions_sorted_by_groundtruth(y_true: List[float], 
    ...
    # Sort y_true indices in ascending order
