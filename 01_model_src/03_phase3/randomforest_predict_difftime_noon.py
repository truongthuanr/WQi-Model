...
def create_morning_noon_same_day_dataset(
    ...
    """
    Build dataset to predict same-day noon alkalinity from morning records.
    Time bucketing from 'Time':
      - hour < 9  -> 'morning'
      - 9 <= hour < 15 -> 'noon'
      - hour >= 15 -> 'afternoon'
    Keep only days with both morning and noon; use the last record within each bucket.
    """
    ...
    work = work[work['time_bucket'].isin(['morning', 'noon'])].copy()
    ...
    noon = work[work['time_bucket'] == 'noon'].groupby(['units', 'Date'], as_index=False).tail(1)

    noon_target_col = f"{column}_noon"
    noon_keep = noon[['units', 'Date', column]].rename(columns={column: noon_target_col})
    ...
    merged = pd.merge(morning_keep, noon_keep, on=['units', 'Date'], how='inner')
    ...
    if 'logger' in globals():
        log_and_flush(logger, f"Merged morning→noon dataset shape: {merged.shape}")
...
def RandomForest_random_cv_joint(
    ...
    log_and_flush(logger, "Random Forest 2 Step - Joint CV for Alkaline Today and Noon")
...
def noname():
    ...
    try:
        ...
        pass
    # Override target to noon alkalinity from morning features
    global output_column
    output_column = ['Độ kiềm_noon']
    # Build morning→noon same-day dataset
