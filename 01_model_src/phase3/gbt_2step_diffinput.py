...
input_col_tomorrow =  ['Độ cứng', 'Độ mặn', 'Nhiệt độ', 'TDS', 'Loại ao', 'Công nghệ nuôi', 'pH', 'Tuổi tôm', 'Độ kiềm tdpred']  # độ kiềm dự đoán sẽ được thêm sau
...
def GBT_random_cv_joint(
    ...
    for train_idx, test_idx in ss.split(X_today):
        ...
        model_tomorrow = GradientBoostingRegressor(n_estimators=100,
                                max_depth=10,
...
def noname():
    ...
    log_path = os.path.join(log_dir, f"gbt2stepdiffinput_{current_time.strftime('%y%m%d-%H%M%S')}.log")
    logger = setup_logger(log_path)
    log_and_flush(logger, "Gradient Boosted Tree 2 Step (diff input)")
    ...
    metrics_result, y_true, y_pred = GBT_random_cv_joint(
                    X_today, X_tomorrow_base, y_today, y_tomorrow,
                    input_col_today, input_col_tomorrow,
                    n_splits=100,
                    test_size=0.3
